...
import { X, Database, CheckCircle2, Save, Server, User, Lock, ShieldCheck } from 'lucide-react';
...
export const EditProjectModal = ({ isOpen, onClose, project, onProjectUpdated }) => {
  ...
  useEffect(() => {
    if (project) {
      ...
      setDbType(project.db_type || 'sqlite');
      setDbHost(project.db_host || 'localhost');
      setDbPort(project.db_port || (project.db_type === 'mysql' ? 3306 : 5432));
      setDbName(project.db_name || 'analytics_db');
      setDbUser(project.db_user || 'read_only_user');
      setDbPassword('');
      setLlmProvider(project.llm_provider || 'ollama');
    }
  }, [project]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await projectsAPI.update(project.id, {
        company_id: user?.company_id || project.company_id || 1,
        name,
        description,
        db_type: dbType,
        db_host: dbHost,
      ...
      onProjectUpdated(resp.data);
      onClose();
    } catch (err) {
      setError(err.response?.data?.detail || 'Loyihani tahrirlashda xatolik yuz berdi');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white border border-mist rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-mist flex items-center justify-between bg-slate-50">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-display font-semibold text-lg text-navy-700">Loyihani Tahrirlash #{project.id}</h3>
          </div>
          ...
          {dbType !== 'sqlite' && (
            <div className="bg-slate-50 border border-mist rounded-2xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-600 mb-1">
                    Yangi Parol (Agar o'zgarish bo'lsa)
                  </label>
                  <div className="relative">
                    <input
                      type="password"
              ...
              <div className="flex items-center space-x-1.5 text-[10px] text-teal font-semibold pt-1">
                ...
                <span>Xavfsizlik: Read-Only foydalanuvchi ulanishi tavsiya etiladi.</span>
              </div>
            </div>
          )}
